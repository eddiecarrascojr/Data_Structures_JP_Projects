...
from lab4.runtime_metric import RuntimeMetric
import re
...
def process_data_files(input_file: TextIO, output_file: TextIO) -> None:
    output_file = None
    ...
    try:
        ...
        arr_values = list(map(int, content))

    except ValueError:
        arr_values = [int(num) for num in re.split(r'\D+', content) if num]
